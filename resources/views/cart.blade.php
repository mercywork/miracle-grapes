@extends('layouts.app')

@section('content')

    
         

          <div class="card" style="width: 18rem;">
  　　　　    <img src="image/black.jpeg" class="card-img-top">
    　
    　         <div class="card-body">
                  <h5 class="card-title">巨峰<br>【 2房 】</h5>
                  <p class="card-text">大人気の巨峰です。約2Kg</p>
                  {!! link_to_route('shop', '注文画面へ', [], ['class' => 'btn btn-lg btn-primary']) !!}
              </div>
          </div>
          
          <div class="card" style="width: 18rem;">
  　　　      <img src="image/shine.jpeg" class="card-img-top">
    　
    　         <div class="card-body">
                  <h5 class="card-title">シャインマスカット<br>【 2房 】</h5>
                  <p class="card-text">少数生産のためお早めにご注文ください。約2Kg</p>
                   {!! link_to_route('shop', '注文画面へ', [], ['class' => 'btn btn-lg btn-primary']) !!}
              </div>
          </div>
      
          <div class="card" style="width: 18rem;">
  　　　　    <img src="image/grapes-set.jpeg" class="card-img-top">
    　
        　     <div class="card-body">
                  <h5 class="card-title">巨峰・<br>シャインマスカット<br>【 1房ずつ 】</h5>
                  <p class="card-text">巨峰もシャインマスカットも欲しい方におすすめ。約2Kg</p>
                   {!! link_to_route('shop', '注文画面へ', [], ['class' => 'btn btn-lg btn-primary']) !!}
              </div>
          </div>
          
       
        
    
    
        </div>
  

@endsection